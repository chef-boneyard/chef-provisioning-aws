  ...
  action :create do
    if !aws_object
      ...
      converge_by "Creating new Subnet #{new_resource.name} with CIDR #{cidr_block} in VPC #{new_resource.vpc} in #{region}" do
        ...
        subnet = aws_driver.ec2.subnets.create(cidr_block, opts)
        subnet.tags['Name'] = new_resource.name
        subnet.tags['VPC'] = new_resource.vpc
        save_managed_entry(id: subnet.id)
        @aws_object = subnet
      end
    end

    if !new_resource.map_public_ip_on_launch.nil?
      subnet_desc = aws_driver.ec2.client.describe_subnets(subnet_ids: [ aws_object.id ])[:subnet_set].first
      if new_resource.map_public_ip_on_launch
        if !subnet_desc[:map_public_ip_on_launch]
          converge_by "Turning on automatic public IPs for subnet #{aws_object.id}" do
            aws_driver.ec2.client.modify_subnet_attribute(subnet_id: aws_object.id, map_public_ip_on_launch: { value: true })
          end
        end
      else
        if subnet_desc[:map_public_ip_on_launch]
          converge_by "Turning off automatic public IPs for subnet #{aws_object.id}" do
            aws_driver.ec2.client.modify_subnet_attribute(subnet_id: aws_object.id, map_public_ip_on_launch: { value: false })
          end
        end
      end
    end
